...
class FloatingView(private val service: LimitService) : LimitView {
    ...
    private fun inflateDebugging() {
        debuggingText = LayoutInflater.from(ContextThemeWrapper(service, R.style.Theme_Velociraptor))
    ...
    private fun inflateMonitor() {
        ...
        models.add(ArcProgressStackView.Model("", 0f,
    ...
    private fun initMonitorPosition() {
        ...
        floatingView!!.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                val split = PrefUtils.getFloatingLocation(service).split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        ...
        if (limitView != null && limitText != null) {
            ...
            when (style) {
                PrefUtils.STYLE_US -> {
                    ...
                    val cardTopBottomPadding = (2 * 1.5 + (1 - Math.cos(Math.toRadians(45.0))) * 2).toFloat()
    ...
    override fun hideLimit(hideLimit: Boolean) {
        limitView!!.visibility = if (!PrefUtils.getShowLimits(service)) View.GONE else if (hideLimit) View.INVISIBLE else View.VISIBLE
    }
